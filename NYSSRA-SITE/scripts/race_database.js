...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadFromURLParams() {
    ...
    if (params.has("search")) document.getElementById("search-box").value = params.get("search");
    if (params.has("start")) document.getElementById("date-start").value = params.get("start");
    if (params.has("end")) document.getElementById("date-end").value = params.get("end");
...
function renderRaces() {
        ...
        <div class="card-body d-flex justify-content-between align-items-center">
          <div style="margin-right: 20px;">
            ...
            <p class="card-text mb-0">
              ...
              <strong>Date:</strong> ${new Date(race.timestamp).toLocaleString()}
            </p>
          </div>
  ...
  Navbar.loadAllRaces().then((json) => {
    ...
    loadFromURLParams(); 
    applyFilters();
    sortRaces(document.getElementById("sort-select").value || "timestamp_desc");
  });
});
