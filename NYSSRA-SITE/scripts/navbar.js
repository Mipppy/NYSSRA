...
class Navbar {
                ...
                return [
                    {
                        ...
                        filter: function (text) {
                            ...
                            text = text.replace(
                                /<table(\s*[^>]*)>/g,
                                '<div class="table-responsive"><table$1 class="table table-striped table-bordered table-hover">'
                            );
    ...
    static simpleMDEParameters = {
        ...
        toolbar: [
            ...
            },
            {
                ...
                action: function (editor) {
                    const cm = editor.codemirror;
                    var date_adjust = parseInt(
                        prompt(
                            `Enter the number of years back or forward you want to show.\nExample: -2 to show ${
                                Globals.get_year() - 2
                            }`,
                            "0"
                        )
                    ...
                    cm.replaceSelection(`<year=${date_adjust}>`);
                    cm.focus();
                },
    ...
    static async LoadExtraHTML() {
        ...
        document.querySelector("nav").innerHTML = await (
            await fetch(`/embeds/navbar.html`)
        ).text(); // These don't want the ts= param because they are safe to cache.
        document.querySelector("footer").innerHTML = await (
            await fetch("/embeds/footer.html")
        ).text();
    }
    ...
    static setGoogleCalendarLink(postData, element) {
        ...
        startDate.setHours(
            postData.advancedEventData.eventStartHour || 0,
            0,
            0
        );
        ...
        const endDate = new Date(
            startDate.getTime() +
                (postData.advancedEventData.eventLength || 0) * 60 * 60 * 1000
        );

        const title = encodeURIComponent(postData.postName);
        const details = encodeURIComponent(
            postData.advancedEventData.eventDescription || ""
        );
        const location = encodeURIComponent(
            postData.advancedEventData.eventLocation || ""
        );
        const recurrence =
            typeof postData.advancedEventData.eventRecurrence === "string" &&
            postData.advancedEventData.eventRecurrence
        ...
        const googleCalendarURL = `
            ...
            ?action=TEMPLATE
            &text=${title}
            &dates=${Navbar.toGoogleCalDate(
                startDate
            )}/${Navbar.toGoogleCalDate(endDate)}
            &details=${details}
            &location=${location}
    ...
    static async getAllTags() {
        const res = await fetch(`${Navbar.url}/get_all_tags`)
        const json = await res.json()
        return json.tags
    }
    ...
    static async loadPageFromURL() {
        ...
        const md_request = await fetch(
            `${Navbar.url}/pages/${article}.md?ts=${Date.now()}`
        ); // Prevent Caching !!!
        const md_text = await md_request.text();

        const article_req = await fetch(
            `${Navbar.url}/page_data/${article}.txt?ts=${Date.now()}`
        ); // Prevent caching!!!
    ...
    static async loadPageFromName(name) {
        ...
        const md_request = await fetch(
            `${Navbar.url}/pages/${new_name}.md?ts=${Date.now()}`
        );
        const md_text = await md_request.text();

        const article_req = await fetch(
            `${Navbar.url}/page_data/${new_name}.txt?ts=${Date.now()}`
        );
        const article_data = await article_req.text();
        const parsed_data = this.parsePageData(article_data);
    ...
    static async loadRaceData(race_filename) {
        const req = await fetch(
            `${Navbar.url}/livetiming_data/${race_filename}?ts=${Date.now()}`
        );
        if (!req.ok) {
            return [];
        }
