...
class DLL_Race_Handler:
        ...
        while True:
            ...
            if self.is_currently_racing:
                ...
                while True:
                    ...
                    if not record:
                        break
                    self.logger.debug("Timer record:", record.as_dict())
    ...
    def _handle_timer_record(self, record: XC_TIMER_DLL.XC_TIMER_RECORD_STRUCTURE_TYPE) -> BatchedTimerRecord | None:
        ...
        if Instances.settings.get_setting("USE_PC_TIME") == 1:
            if record.record_typ == 'b':
                ...
                self.logger.debug(f"New timer record {active_batched_record}.")
    ...
    def start_race(self, data: dict) -> int:
        """
        Starts a race.
        ...
        """
        if self.startlist:
            self.started_race_data = data
            self.logger.debug(f"Starting Race with Data: {data}")
            self.logger.info(int(data.get("commPort", "COM3").replace('COM', '')))
            race_dir = os.path.abspath("srt")
            self.xc_timer_dll.dll_initialize_dll_task(0x100 | 0x40 | 0x10 | 2, race_dir)
            ...
            # Event and heat are effectively meaningless.
            self.li = random.sample(range(1, 8), 2)
            self.started_race_data.update({"event": self.li[0], "heat": self.li[1]})
            self.xc_timer_dll.dll_set_event_and_heat(0, self.li[0], self.li[1])
            ...
            self.logger.critical('Attempted to start a race without a startlist loaded.  This should be impossible, try restart/reinstalling the program.')
            return -1
    ...
    def end_race(self):
        ...
        self.is_currently_racing = False
        self.save_results()
        self.started_race_data = {}
        from instances import Instances
        Instances.window.bridge.send_to_js("RACE_ENDED|||yeah_it_did_woah")
